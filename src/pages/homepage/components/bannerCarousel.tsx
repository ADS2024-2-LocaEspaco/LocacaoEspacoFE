...
const Carrossel: React.FC<CarrosselProps> = ({ anuncios }) => {
  ...
  return (
    <div className="relative">
      {/* Contêiner do carrossel com margens de 128px nas laterais */}
      <div
        className="overflow-hidden rounded-[42px] sm:rounded-[26px]"
        ref={emblaRef}
        ...
        <div className="flex h-[574px] justify-stretch items-stretch">
          {imagens.map((imagem, index) => (
            <div key={index} className="flex-[0_0_100%] relative">
              ...
              <div className="absolute top-10 left-14 bg-white bg-opacity-75 text-black-100 px-3 py-1 rounded-md shadow-md z-20">
                <span className="text-sm font-semibold">{titulo} - {endereco}</span>
              </div>

              {/* Gradiente de fundo */}
              <div className="sticky z-10 bottom-0 left-0 w-full h-[264px] rounded-b-lg h-14 bg-gradient-to-t from-[#021125]"></div>
      ...
      <h2 className="absolute z-20 -ml-[80px] bottom-[36px] left-[140px] text-[20px] sm:text-5xl text-white font-bold">Melhores avaliados</h2>

      {/* Botão "Ver local" */}
      <div className="absolute bottom-12 right-0 transform -translate-x-1/2">
        <button className="bg-orange-300 text-white px-6 py-2 rounded-[50px] shadow hover:bg-orange-400 z-30 hidden sm:block"
