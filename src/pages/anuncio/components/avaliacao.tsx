...
import { AvaliacaoProps, MediaProps } from '@/types/types2';
...
const Avaliacao: React.FC<AvaliacaoProps> = (avaliacoes) => {
  const [notaMedia, setNotaMedia] = useState<number>(0);
console.log('Avaliações no compo Avaliação: ', avaliacoes)
  useEffect(() => {
    if (avaliacoes?.media_notas) {
      const notas = avaliacoes.media_notas;
      const valores = [
