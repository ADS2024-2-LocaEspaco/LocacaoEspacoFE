      ...
      {isDesktop ? (
        <div className="grid grid-cols-3 gap-0 max-h-[400px] mt-5">
          <div className="grid grid-rows-2 gap-4 content-center p-2 m-0">
            {imagens.slice(0, 2).map((imagem, index) => (
              <img
                key={imagem.id}
                src={imagem.url}
                alt={`Imagem ${index + 1}`}
          ...
          <div className="grid grid-rows-2 gap-4 content-center p-2 m-0">
            {imagens.slice(3, 5).map((imagem, index) => (
              <img
                key={imagem.id}
                src={imagem.url}
                alt={`Imagem ${index + 3}`}
        ...
        >
          <div className="flex justify-stretch items-stretch">
            {imagens.map((imagem, index) => (
              <div key={imagem.id} className="flex-[0_0_100%]">
                <img
                  src={imagem.url}
                  alt={`Imagem ${index + 1}`}
      ...
      >
        <div className="relative flex items-center justify-center w-full h-full">
          ...
          >
            <div className="flex">
              {imagens.map((imagem, index) => (
                <div key={imagem.id} className="z-10 flex-[0_0_100%]">
                  <img
                    src={imagem.url}
                    alt={`Imagem ${index + 1}`}
